...
; SHADERTEST: [[itop:%[0-9]*]] = inttoptr i64 [[bc]] to <{ i32 }> addrspace(1)*

; SHADERTEST: [[ld1:%[0-9]*]] = load i64, i64 addrspace(5)*
; SHADERTEST: [[itop:%[0-9]*]] = inttoptr i64 [[ld1]] to <{ i32 }> addrspace(1)*

; SHADERTEST: [[int:%[0-9]*]] = ptrtoint <{ i32 }> addrspace(1)* {{%[0-9]*}} to i64
; SHADERTEST: [[bc2:%[0-9]*]] = bitcast i64 [[int]] to <2 x i32>

; SHADERTEST: [[ld2:%[0-9]*]] = load <{ i32 }> addrspace(1)*, <{ i32 }> addrspace(1)* addrspace(5)*
; SHADERTEST: [[ptoi:%[0-9]*]] = ptrtoint <{ i32 }> addrspace(1)* [[ld2]] to i64
